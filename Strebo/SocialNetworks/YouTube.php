...
class YouTube extends Strebo\AbstractSocialNetwork implements Strebo\PrivateInterface, Strebo\PublicInterface
{
    ...
    {
        return $this->encodeJSON($this->youtube->search->listSearch("player,snippet,statistics", ["maxResults" => 10, "q" => $tag]));
    }

    public function getPublicFeed($location)
    {
        if ($location != null) {
            $popularMedia = $this->youtube->videos->listVideos("player,snippet,statistics", ["chart" => "mostPopular", "regionCode" => $location, "maxResults" => 10]);
        } else {
            $popularMedia = $this->youtube->videos->listVideos("player,snippet,statistics", ["chart" => "mostPopular", "maxResults" => 10]);
        }
    ...
    public function encodeJSON($json)
    {
        ...
        foreach ($json->items as $item) {
            ...
            $data['media'] = $item->player->embedHtml;
            $data['thumb'] = $item->snippet->thumbnails->standard->url;
            $data['text'] = $item->snippet->title;
